  ...
  <div id="f1-hud-panel">
      ...
      ">
        ...
        <svg width="58" height="42" viewBox="0 0 58 42" id="f1-aero-svg">
          <defs>
            ...
            <style>
              .f1-rw-flap { transform-box:fill-box; transform-origin:0% 50%; transition:transform .4s cubic-bezier(.4,0,.2,1); }
    ...
    <div style="display:flex;align-items:center;gap:5px;">
      ...
      <div style="position:relative;width:160px;height:160px;flex-shrink:0;">
        <svg width="160" height="160" style="position:absolute;top:0;left:0;" id="f1-circle-svg">
          <defs>
          ...
          <circle cx="80" cy="80" r="74" fill="none" stroke="#030c0c" stroke-width="10"/>
          <circle cx="80" cy="80" r="68" fill="none" stroke="#061010" stroke-width="1"/>
          <circle cx="80" cy="80" r="68" fill="none" stroke="#041212" stroke-width="8"
            stroke-dasharray="${(2*Math.PI*68*.8).toFixed(1)} ${(2*Math.PI*68*.2).toFixed(1)}"
            transform="rotate(126 80 80)"/>
          <!-- Glow arc -->
          <circle id="f1-arc-glow" cx="80" cy="80" r="68" fill="none"
            stroke="#0dcfcf" stroke-width="16" stroke-opacity=".10" stroke-linecap="round"
            stroke-dasharray="0 ${(2*Math.PI*68).toFixed(1)}"
            transform="rotate(126 80 80)" style="transition:stroke-dasharray .3s,stroke .5s;"/>
          <!-- Main arc -->
          <circle id="f1-arc-main" cx="80" cy="80" r="68" fill="none"
            stroke="#0dcfcf" stroke-width="6" stroke-linecap="round"
            stroke-dasharray="0 ${(2*Math.PI*68).toFixed(1)}"
            transform="rotate(126 80 80)" filter="url(#tg2)"
            style="transition:stroke-dasharray .3s,stroke .5s;"/>
          <circle cx="80" cy="80" r="59" fill="url(#cbg2)"/>
          <circle cx="80" cy="80" r="59" fill="none" stroke="#041010" stroke-width="1.2"/>
        </svg>
        <!-- Circle text overlay -->
        <div style="
        ...
        ">
          <span style="font-family:'Barlow Condensed',sans-serif;font-size:8px;font-weight:700;letter-spacing:3px;color:#0dcfcf;">KM/H</span>
          <span id="f1-kmh" style="font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:900;line-height:1;color:#fff;text-shadow:0 0 16px rgba(255,255,255,.2);">0</span>

          <!-- BOOST label -->
          <div id="f1-boost-label-box" style="padding:1px 10px;border-radius:1px;border:1px solid transparent;transition:all .3s;margin-top:2px;">
            <span id="f1-boost-label" style="font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:900;letter-spacing:4px;color:#0a2a1a;transition:all .3s;">BOOST</span>
          </div>
          <!-- OVERTAKE label -->
          <div id="f1-overtake-label-box" style="padding:1px 4px;border-radius:1px;border:1px solid transparent;transition:all .3s;">
            <span id="f1-overtake-label" style="font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:900;letter-spacing:4px;color:#0a2a1a;transition:all .3s;">OVERTAKE</span>
          </div>

          <span id="f1-mph" style="font-family:'Barlow Condensed',sans-serif;font-size:30px;font-weight:900;line-height:1;color:#ccc;margin-top:1px;">0</span>
          <span style="font-family:'Barlow Condensed',sans-serif;font-size:8px;font-weight:700;letter-spacing:3px;color:#0dcfcf;">MPH</span>

          <!-- Driver full name -->
          <span id="f1-circle-driver" style="
            font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:900;
            letter-spacing:4px;color:#0dcfcf;text-shadow:0 0 10px #0dcfcf;
            margin-top:2px;max-width:130px;text-align:center;
            overflow:hidden;white-space:nowrap;text-overflow:ellipsis;
            transition:color .5s,text-shadow .5s;
          ">DRIVER</span>

          <!-- Battery -->
          <svg id="f1-battery-svg" width="26" height="12" viewBox="0 0 26 12" style="margin-top:3px;">
            <rect x="0" y="1" width="22" height="10" rx="2" fill="none" stroke="#0dcfcf" stroke-width="1"/>
            <rect x="22" y="4" width="3" height="4" rx="1" fill="#0dcfcf"/>
            <rect id="f1-battery-fill" x="1.5" y="3" width="0" height="6" rx="1" fill="#00ff88" style="transition:width .25s,fill .5s;"/>
          </svg>
        </div>
      </div>
...
function renderWidget() {
  ...
  if (arcMain) {
    arcMain.style.strokeDasharray = `${arcFill * .8} ${C}`;
    arcMain.setAttribute('stroke', mc.arc);
  }
  if (arcGlow && arcFill > 0.5) {
    arcGlow.style.strokeDasharray = `${arcFill * .8} ${C}`;
    arcGlow.setAttribute('stroke', mc.arc);
  }
  ...
  setText('f1-circle-driver',   state.driver.toUpperCase());
  ...
  setStyle('f1-circle-driver',   'color', mc.arc);
  setStyle('f1-circle-driver',   'textShadow', `0 0 10px ${mc.arc}`);
  ...
  // ── Battery ──
  const bw = Math.round((state.energy / 100) * 19);
  ...
  if (rampLabel) rampLabel.style.color  = state.kmh > RAMP_LO ? rampClr : '#0a4040';
  ...
  if (deratingBox) {
    deratingBox.style.borderColor  = derating ? '#FF444460' : '#0a1414';
    deratingBox.style.background   = derating ? 'rgba(255,68,68,0.08)' : 'transparent';
  }
  if (deratingDot) {
    deratingDot.style.background  = derating ? '#FF4444' : '#0a1c1c';
    deratingDot.style.boxShadow   = derating ? '0 0 6px #FF4444' : 'none';
    deratingDot.style.animation   = derating ? 'f1-blink .9s ease-in-out infinite' : 'none';
  }
  if (deratingLbl) {
    deratingLbl.textContent = derating ? 'DERATING' : 'NOMINAL';
  ...
  ['f1-rw-pivot','f1-fw-pivot'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.setAttribute('fill', aeroColor);
  });
  ['f1-rw-rect','f1-fw-rect'].forEach(id => {
    const el = document.getElementById(id);
  ...
  if (aeroStateLabel) {
    aeroStateLabel.textContent = straight ? 'FLAT · LOW DRAG' : 'ANGLED · DOWNFORCE';
    aeroStateLabel.setAttribute('fill', `${aeroColor}99`);
  }
  if (aeroModeLabel) {
    aeroModeLabel.textContent  = straight ? 'X-MODE' : 'Z-MODE';
    aeroModeLabel.style.color  = aeroColor;
    aeroModeLabel.style.textShadow = `0 0 8px ${aeroColor}`;
  }
}
...
"gear"
  ...
  Infer from speed and situation:
  - Gear 1: 0–80 km/h
  - Gear 2: 80–130 km/h
  - Gear 3: 130–175 km/h
  - Gear 4: 175–215 km/h
  ...
  - Gear 7: 295–330 km/h
  - Gear 8: 330–370 km/h
  Under heavy braking: descend through gears rapidly.

"mode"
  One of: "NORMAL", "BOOST", "OVERTAKE".
  - OVERTAKE: driver is within 1 second of car ahead and has activated the button, or text explicitly says overtake mode / OT / pressing the button to attack.
  - BOOST: driver is defending, pushing hard, or text mentions boost / extra power burst without being in an overtake situation.
  - NORMAL: default — cruising, managing, no mode active.

"aero"
  "X" if on a straight (wings flat, low drag mode).
  "Z" if in a corner or braking zone (wings angled, high downforce).
  Default to "X" on straights, "Z" everywhere else.

"deploy"
  Integer 0–5. ERS segments being deployed (energy going OUT).
  - 0: harvesting only, no deployment
  - 1–2: light deployment, managing
  - 3–4: meaningful deployment, attacking or defending
  - 5: maximum deployment, Overtake Mode or desperate push
  High deploy = energy store draining.

"recharge"
  Integer 0–5. ERS segments being harvested (energy coming IN).
  - 0: no harvesting (full deployment straight)
  - 1–2: light harvest alongside deployment (super-clipping)
  - 3–5: active harvesting under braking or lift-and-coast
  High recharge = under braking or coasting, energy store refilling.

"energy"
  Integer 0–100. Percentage of energy store remaining.
  Decreases when deploy > recharge. Increases under heavy braking.
  If not explicitly mentioned, estimate from context:
  - Early race / fresh lap: 80–100
  - Mid-stint after multiple OT activations: 40–70
  - End of long straight with heavy deployment: 20–50
  - After derating warning: 10–25

"brake"
  Integer 0–100. Brake pressure percentage.
  - 0: full throttle, no braking
  - 10–30: light trail braking mid-corner
  - 50–80: medium braking zone
  - 90–100: heavy braking zone (end of long straight into slow corner)

"throttle"
  Integer 0–100. Throttle percentage.
  - 0: full braking, no throttle
  - 20–50: partial throttle mid-corner
  - 70–90: corner exit acceleration
  - 95–100: full throttle on straight
  Brake and throttle should not both be high simultaneously.

"turn"
  Integer. The corner number currently being navigated or most recently mentioned.
  Keep the previous value if no new corner is mentioned.
  Default 1 if completely unknown.
...
async function parseContext() {
  ...
  try {
    ...
    // Validate and apply — only update fields that came back valid
    ...
    console.log(`[${EXT_NAME}] Parsed telemetry:`, parsed);
  } catch (err) {
...
function registerEvents() {
  // Fire parser after every AI message received
  eventSource.on(event_types.MESSAGE_RECEIVED, () => {
    // Small delay so the message is fully written to chat before we read it
    setTimeout(parseContext, 300);
  });

  // Also fire after user sends (for slash command responsiveness)
  eventSource.on(event_types.MESSAGE_SENT, () => {
    setTimeout(parseContext, 300);
  });

  console.log(`[${EXT_NAME}] ST events registered.`);
}
...
(function init() {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { injectWidget(); registerEvents(); });
  } else {
    injectWidget();
    registerEvents();
  }
