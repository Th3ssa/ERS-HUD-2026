    ...
    try {
      if (window.SillyTavern && typeof window.SillyTavern.getContext === 'function') {
        ...
        if (ctx && ctx.chat && ctx.chat.length > 0) {
          console.log('[' + ID + '] getChat: using SillyTavern.getContext(), ' + ctx.chat.length + ' msgs');
          return { chat: ctx.chat, name2: ctx.name2 || '' };
        }
      }
    } catch (e) { console.warn('[' + ID + '] SillyTavern.getContext() failed:', e.message); }
    const chat = window.chat || [];
    const name2 = window.name2 || (window.characters && window.this_chid !== undefined ? (window.characters[window.this_chid] || {}).name : '') || '';
    console.log('[' + ID + '] getChat: using window.chat, ' + chat.length + ' msgs, char=' + name2);
    return { chat: chat, name2: name2 };
  }

  async function runQuietPrompt(prompt) {
    // Path 0: window.generateQuietPrompt (most common)
    if (typeof window.generateQuietPrompt === 'function') {
      console.log('[' + ID + '] Using window.generateQuietPrompt');
      return await window.generateQuietPrompt(prompt, false, false);
    }
    // Path 1: SillyTavern namespace
    if (window.SillyTavern && typeof window.SillyTavern.generateQuietPrompt === 'function') {
      console.log('[' + ID + '] Using SillyTavern.generateQuietPrompt');
      return await window.SillyTavern.generateQuietPrompt(prompt, false, false);
    }
    // Path 2: extension_helper if exposed
    if (window.extension_helper && typeof window.extension_helper.generateQuietPrompt === 'function') {
      console.log('[' + ID + '] Using extension_helper.generateQuietPrompt');
      return await window.extension_helper.generateQuietPrompt(prompt, false, false);
    }
    // Log what IS on window for debugging
    console.error('[' + ID + '] generateQuietPrompt not found. Available window keys with "generate":', 
      Object.keys(window).filter(function(k){ return k.toLowerCase().includes('generate') || k.toLowerCase().includes('quiet'); })
    );
    throw new Error('generateQuietPrompt not available — see console for debug info');
  }
...
<div id="f1-hud-handle">
  ...
  function inject() {
    ...
    // ── Position helper ──
    // Always pin the RIGHT edge of the widget to a fixed right offset,
    // so expanding the panel leftward never causes overflow.
    // We store rightOffset and topOffset, and recompute left on every render.
    let rightOffset = 0;   // px from right edge of screen
    let topOffset   = 0;   // px from top of screen

    function applyPosition() {
      const w = root.offsetWidth || 28;
      root.style.left = Math.max(0, window.innerWidth - w - rightOffset) + 'px';
      root.style.top  = Math.max(0, Math.min(window.innerHeight - root.offsetHeight, topOffset)) + 'px';
    }

    root.style.right = 'auto';
    // Give DOM a tick to measure offsetWidth after panel is hidden
    ...
    // Tags that should pass through drag without triggering it
    const INTERACTIVE = ['BUTTON','INPUT','SELECT','TEXTAREA','A'];

    function onPointerDown(e) {
      // Don't start drag on interactive elements
      if (INTERACTIVE.indexOf(e.target.tagName) !== -1) return;
      ...
      startX = t.clientX;
      startY = t.clientY;
      // Store current right/top offsets as drag start reference
    ...
    function onPointerMove(e) {
      ...
      if (!dragMoved) return;
      // Moving right = smaller rightOffset, moving left = larger rightOffset
      rightOffset = Math.max(0, Math.min(window.innerWidth  - root.offsetWidth,  startRight - dx));
      topOffset   = Math.max(0, Math.min(window.innerHeight - root.offsetHeight, startTop   + dy));
      applyPosition();
    }
    ...
    function onPointerUp(e) {
      ...
      // Mobile tap = toggle (only if it was a clean tap on the handle)
      if (!dragMoved && e && e.type === 'touchend') {
        const onHandle = e.target && e.target.closest && e.target.closest('#f1-hud-handle');
        if (onHandle) { state.open = !state.open; applyPosition(); render(); }
      }
    }

    // Attach to the ROOT so the whole widget surface is draggable
    ...
    // Desktop click toggle (handle only)
    const handle = document.getElementById('f1-hud-handle');
    handle.addEventListener('click', function () {
      if (dragMoved) return;
      state.open = !state.open;
      // Reposition after panel opens/closes so right edge stays anchored
      setTimeout(applyPosition, 0);
      render();
    });

    document.getElementById('f1-collapse-btn').addEventListener('click', function (e) {
      e.stopPropagation();
      state.open = false;
      setTimeout(applyPosition, 0);
      render();
